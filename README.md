...
> Copyright (c) 2026 KaiTSWEBSITE. All rights reserved.
>
> This repository is proprietary software. Do not copy, redistribute, resell,
> re-upload, rebrand, or reuse this source code without written permission from
> KaiTSWEBSITE. See [LICENSE](./LICENSE), [COPYRIGHT.md](./COPYRIGHT.md), and
> [NOTICE.md](./NOTICE.md).
...
- Tài khoản admin bị chặn khỏi form login công khai; phải đi qua cổng riêng `/eden-vault` với `ADMIN_ACCESS_KEY`.
...
### Bật CAPTCHA khi vào web

EdenVerse dùng Cloudflare Turnstile để chặn bot nhẹ và mượt hơn reCAPTCHA. Tạo Turnstile site trên Cloudflare, sau đó thêm các biến này vào Vercel:

```env
NEXT_PUBLIC_CAPTCHA_ENABLED="true"
NEXT_PUBLIC_TURNSTILE_SITE_KEY="site_key_cong_khai"
TURNSTILE_SECRET_KEY="secret_key_bao_mat"
```

`NEXT_PUBLIC_TURNSTILE_SITE_KEY` là public nên người khác có thể thấy trong source web. `TURNSTILE_SECRET_KEY` phải chỉ nằm trong Vercel/server, không đưa lên GitHub.
...
- `ADMIN_ACCESS_KEY`: mã cổng riêng để cho phép tài khoản admin đăng nhập qua `/eden-vault`.
...
5. Tạo `ADMIN_ACCESS_KEY` mạnh trong Vercel và chỉ chủ sở hữu biết mã này.
6. Đăng nhập bằng tài khoản Super Admin thật tại `/eden-vault`, sau đó đổi mật khẩu trong `/admin`.
...
- `/auth/login`: đăng nhập người dùng thường bằng email/password.
- `/eden-vault`: cổng riêng cho chủ sở hữu/admin, yêu cầu thêm `ADMIN_ACCESS_KEY`.
