...
// SVG is intentionally excluded — it can embed <script> tags and cause XSS
...
/**
 * Validates that the resolved upload path stays within the allowed upload directory.
 * Prevents path traversal attacks (e.g., filenames containing "../../../etc/passwd").
 */
function isSafeUploadPath(uploadDir: string, fileName: string): boolean {
  const resolved = path.resolve(uploadDir, fileName);
  return resolved.startsWith(path.resolve(uploadDir) + path.sep);
}
...
export async function POST(request: Request) {
  ...
  if (file.type === "image/svg+xml" || file.name.toLowerCase().endsWith(".svg")) {
    return NextResponse.json(
      { message: "SVG không được phép upload vì lý do bảo mật. Dùng PNG hoặc WEBP thay thế." },
      { status: 415 }
    );
  }
  ...
  if (!isSafeUploadPath(uploadDir, safeName)) {
    return NextResponse.json({ message: "Tên tệp không hợp lệ." }, { status: 400 });
  }
