...
type DownloadMirror = "primary" | "backup" | "joyplay" | "season2";
  ...
  return (
    <div className="space-y-2">
      <div className="flex flex-wrap gap-3">
        ...
        {hasJoyplay ? (
          <Button variant="secondary" onClick={() => openDownload("joyplay")} disabled={Boolean(pendingMirror)}>
            <Download className="h-4 w-4" />
            {pendingMirror === "joyplay" ? "Đang mở JoyPlay..." : "Tải JoyPlay"}
          </Button>
        ) : null}
        {hasSeason2 ? (
          <Button variant="secondary" onClick={() => openDownload("season2")} disabled={Boolean(pendingMirror)}>
            <Download className="h-4 w-4" />
            {pendingMirror === "season2" ? "Đang mở Season 2..." : "Tải Season 2"}
          </Button>
        ) : null}
      </div>
